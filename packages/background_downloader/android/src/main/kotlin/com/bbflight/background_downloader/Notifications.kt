...
object NotificationService {
    ...
    suspend fun updateNotification(
        ...
        // MBNime: artwork is cached before enqueue, so native background workers
        // can render it without depending on a live Flutter engine or networking.
        try {
            val coverPath = org.json.JSONObject(taskWorker.task.metaData).optString("coverPath")
            val cover = java.io.File(coverPath).canonicalFile
            val allowed = java.io.File(taskWorker.appContext.filesDir, "download-covers").canonicalFile
            if (cover.path.startsWith(allowed.path + java.io.File.separator) &&
                cover.isFile && cover.length() <= 2 * 1024 * 1024) {
                val bounds = android.graphics.BitmapFactory.Options().apply { inJustDecodeBounds = true }
                android.graphics.BitmapFactory.decodeFile(cover.path, bounds)
                val options = android.graphics.BitmapFactory.Options().apply {
                    inSampleSize = maxOf(1, maxOf(bounds.outWidth, bounds.outHeight) / 192)
                }
                android.graphics.BitmapFactory.decodeFile(cover.path, options)?.let { builder.setLargeIcon(it) }
            }
        } catch (_: Exception) { /* Artwork is optional; transfers must continue. */ }
